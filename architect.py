...
class Architect():
    ...
    def virtual_step(self, trn_X, trn_y, lr, w_optim):
        ...
        # compute gradient for L_{train}
        gradients = torch.autograd.grad(loss, self.net.weights()) # Eq.(4)

        # Virtual step (update gradient)
        with torch.no_grad():
            for w, vw, g in zip(self.net.weights(), self.v_net.weights(), gradients):
                m = w_optim.state[w].get('momentum_buffer', 0.) * self.w_momentum
                vw.copy_(w - lr * (m + g + self.w_weight_decay*w)) # Eq.(6)
    ...
    def unrolled_backward(self, trn_X, trn_y, val_X, val_y, lr, w_optim):
        ...
        # update final gradient = dalpha - lr*hessian # Eq.(6)
        with torch.no_grad(): 
    ...
    def compute_hessian(self, dw, trn_X, trn_y):
        """
        Eq.(8)
        In original paper, instead of computing directly hessian matirx, it computes finite difference.
        dw = dw` { L_val(w`, alpha) }
        w+ = w + eps * dw
        w- = w - eps * dw
